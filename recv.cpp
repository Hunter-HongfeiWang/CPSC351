...
string recvFileName()
{
    ...
    if(msgrcv(msqid, &msgName, sizeof(fileNameMsg) - sizeof(long), FILE_NAME_TRANSFER_TYPE, 0) == -1)
    {
        perror("msgsnd");
        exit(-1);
    }
...
void init(int& shmid, int& msqid, void*& sharedMemPtr)
{
	...
	*/
    ...
    shmid = shmget(key,SHARED_MEMORY_CHUNK_SIZE, 0664|IPC_CREAT);
       ...
       msgSize = msg.size;
		...
		if(msgSize != 0)
		{
			 ...
			 * of type ackMessage with mtype field set to RECV_DONE_TYPE. 
 			 */
            		ackMessage msgRecvd;
            		msgRecvd.mtype = RECV_DONE_TYPE;
            		if(msgsnd(msqid, &msgRecvd, sizeof(ackMessage) - sizeof(long),0 ) == -1)
            		{
                		perror("msgsnd");
                		exit(-1);
            		}
